 ...
 * @param cwd
 * @param pkg
 * @returns narcissus-fe-eslint-config/index
 * @returns narcissus-fe-eslint-config/react
 * @returns narcissus-fe-eslint-config/typescript/index
 * @returns narcissus-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'narcissus-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
